...
async def run(DATA, can, lcd=None):
    ...
    print("CRANK(IO): Forcing CAN1 re-init for cold-boot…")
    try:
        can.hwcan.init(can.MODE_NORMAL, 500000)
    except Exception as e:
        print("CRANK(IO): CAN re-init skipped or unsupported:", e)
    ...
    for i in range(cfg["tpdo_retries"]):
        print("  waiting for TPDO2… (%d/%d)" % (i+1, cfg["tpdo_retries"]))
    ...
    # ------------------------------------------------------------------
    print("CRANK(IO): ramping throttle…")
